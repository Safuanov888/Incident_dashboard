...
selected_departments = st.sidebar.multiselect('Выберите отдел', employees['department'].unique(),
                                              default=employees['department'].unique())
selected_access = st.sidebar.multiselect('Уровень доступа', employees['access_logs'].unique(),
                                         default=employees['access_logs'].unique())
...
fig1 = px.bar(filtered_employees, x='department', color='department',
              title='Количество сотрудников в зависимости от отдела')
...
groupby_failure = filtered_logs[filtered_logs['status'] == 'failure'].groupby(pd.Grouper(key='date', freq='W'))[
    'status'].count()
...
st.dataframe(data=table, width='stretch', height='stretch')
